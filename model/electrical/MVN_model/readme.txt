...
Autolaunch from ModelDB and select figures by pressing a button or download and expand the 
archive.  Then on
...
-----------------------------------------
Note that the jpg file in this collection that shows the steady state version of figure
3.  This figure differs slightly from the demo because the demo doesn't reach a steady
state before the currents are displayed.  To get the same currents merely increase the
simulation length (tstop) to greater than 1000 ms and press init and run button.

Note that in the ca_X.mod files the K_ are converted into K2f_'s from the 
paper by multiplying 2*FARADAY them (twice for the charge on Ca).  This effectively
turns the formula's for the j_X fluxes (particles passing boundaries) into formula's 
for i_X currents (coulombs per second).

For questions about the paper please contact Thomas Knopfel, for questions about the model code,
Tom Morse.
