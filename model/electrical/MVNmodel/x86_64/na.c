...
extern Memb_func* memb_func;
 ...
 "alphah_na", _hoc_alphah,
 "alpham_na", _hoc_alpham,
 "betah_na", _hoc_betah,
 "betam_na", _hoc_betam,
 "rates_na", _hoc_rates,
 "tauh_na", _hoc_tauh,
...
#define alphah alphah_na
#define alpham alpham_na
#define betah betah_na
#define betam betam_na
#define tauh tauh_na
#define taum taum_na
...
#define tauh_min tauh_min_na
 double tauh_min = 0.3;
#define taum_min taum_min_na
 double taum_min = 0.05;
 ...
 "taum_min_na", &taum_min_na,
 "tauh_min_na", &tauh_min_na,
 0,0
};
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 0, "cvodeieq");
 	...
 	ivoc_help("help ?1 na /Users/xiey/Dropbox/GitHub/multiscale-synaptic-model/model/electrical/MVNmodel/x86_64/na.mod\n");
...
double alpham ( _threadargsprotocomma_ double _lVm ) {
   double _lalpham;
  _lalpham = 5.0 * exp ( 0.1 * ( _lVm + 39.5 ) ) ;
...
double betam ( _threadargsprotocomma_ double _lVm ) {
   double _lbetam;
  _lbetam = 5.9 * exp ( - 0.044 * ( _lVm + 39.5 ) ) ;
...
double alphah ( _threadargsprotocomma_ double _lVm ) {
   double _lalphah;
  _lalphah = 0.567 * exp ( - 0.024 * ( _lVm + 35.0 ) ) ;
...
double betah ( _threadargsprotocomma_ double _lVm ) {
   double _lbetah;
  _lbetah = 0.567 * exp ( 0.275 * ( _lVm + 35.0 ) ) ;
...
double taum ( _threadargsprotocomma_ double _lVm ) {
   double _ltaum;
  _ltaum = 1.0 / ( alpham ( _threadargscomma_ _lVm ) + betam ( _threadargscomma_ _lVm ) ) ;
   if ( _ltaum < taum_min ) {
     _ltaum = taum_min ;
     }
...
double tauh ( _threadargsprotocomma_ double _lVm ) {
   double _ltauh;
  _ltauh = 1.0 / ( alphah ( _threadargscomma_ _lVm ) + betah ( _threadargscomma_ _lVm ) ) ;
   if ( _ltauh < tauh_min ) {
     _ltauh = tauh_min ;
     }
...
static int  rates ( _threadargsprotocomma_ double _lVm ) {
   ...
   minf = alpham ( _threadargscomma_ _lVm ) / ( alpham ( _threadargscomma_ _lVm ) + betam ( _threadargscomma_ _lVm ) ) ;
   hinf = alphah ( _threadargscomma_ _lVm ) / ( alphah ( _threadargscomma_ _lVm ) + betah ( _threadargscomma_ _lVm ) ) ;
    return 0; }
