...
extern Memb_func* memb_func;
 ...
 "alphab_a", _hoc_alphab,
 "alphaa_a", _hoc_alphaa,
 "betab_a", _hoc_betab,
 "betaa_a", _hoc_betaa,
 "rates_a", _hoc_rates,
 "taub_a", _hoc_taub,
...
#define alphab alphab_a
#define alphaa alphaa_a
#define betab betab_a
#define betaa betaa_a
#define taub taub_a
#define taua taua_a
...
#define taub_min taub_min_a
 double taub_min = 24;
#define taua_min taua_min_a
 double taua_min = 1;
 ...
 "taua_min_a", &taua_min_a,
 "taub_min_a", &taub_min_a,
 0,0
};
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 0, "cvodeieq");
 	...
 	ivoc_help("help ?1 a /Users/xiey/Dropbox/GitHub/multiscale-synaptic-model/model/electrical/MVNmodel/x86_64/a.mod\n");
...
/*CVODE*/
 ...
 static int states (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) { {
   rates ( _threadargscomma_ v ) ;
    a = a + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / tau_a)))*(- ( ( ( ainf ) ) / tau_a ) / ( ( ( ( - 1.0 ) ) ) / tau_a ) - a) ;
    b = b + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / tau_b)))*(- ( ( ( binf ) ) / tau_b ) / ( ( ( ( - 1.0 ) ) ) / tau_b ) - b) ;
   }
  return 0;
}
 
double alphaa ( _threadargsprotocomma_ double _lVm ) {
   double _lalphaa;
  _lalphaa = 0.2 * exp ( 0.14 * ( _lVm + 65.0 ) ) ;
...
double betaa ( _threadargsprotocomma_ double _lVm ) {
   double _lbetaa;
  _lbetaa = 0.2 * exp ( - 0.035 * ( _lVm + 65.0 ) ) ;
...
double taua ( _threadargsprotocomma_ double _lVm ) {
   double _ltaua;
  _ltaua = 1.0 / ( alphaa ( _threadargscomma_ _lVm ) + betaa ( _threadargscomma_ _lVm ) ) ;
   if ( _ltaua < taua_min ) {
     _ltaua = taua_min ;
     }
...
double alphab ( _threadargsprotocomma_ double _lVm ) {
   double _lalphab;
  _lalphab = 0.01 * exp ( - 0.11 * ( _lVm + 71.0 ) ) ;
...
double betab ( _threadargsprotocomma_ double _lVm ) {
   double _lbetab;
  _lbetab = 0.01 * exp ( 0.164 * ( _lVm + 71.0 ) ) ;
...
double taub ( _threadargsprotocomma_ double _lVm ) {
   double _ltaub;
  _ltaub = 1.0 / ( alphab ( _threadargscomma_ _lVm ) + betab ( _threadargscomma_ _lVm ) ) ;
   if ( _ltaub < taub_min ) {
     _ltaub = taub_min ;
     }
...
static int  rates ( _threadargsprotocomma_ double _lVm ) {
   tau_a = taua ( _threadargscomma_ _lVm ) ;
   ainf = alphaa ( _threadargscomma_ _lVm ) / ( alphaa ( _threadargscomma_ _lVm ) + betaa ( _threadargscomma_ _lVm ) ) ;
   tau_b = taub ( _threadargscomma_ _lVm ) ;
   binf = alphab ( _threadargscomma_ _lVm ) / ( alphab ( _threadargscomma_ _lVm ) + betab ( _threadargscomma_ _lVm ) ) ;
    return 0; }
