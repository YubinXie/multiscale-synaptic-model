...
strength_files = sorted(glob.glob('./Duration-H/*/outout.csv'))
...
np.savetxt('duration_ca_ampar.csv',output,fmt="%d,%e,%e", header='Hyperpolarisation duration (ms),Ca,AMPAR', comments='')
...
for i,file in (enumerate([strength_files[x] for x in order])):
	...
	plt.plot(df.index[::sample_rate]/1000,df['Values[AMPAR_bar]'][::sample_rate],c=colors[i],label =sort_condition_names[i],linewidth=3,alpha=1 )
	#break1
...
plt.tight_layout()
plt.savefig('duration_figure_ampar.pdf')
...
for i,file in (enumerate([strength_files[x] for x in order])):
	...
	plt.plot(df.index[::sample_rate]/1000,df['Values[CaMKII_active_ratio]'][::sample_rate],c=colors[i],label =sort_condition_names[i],linewidth=3,alpha=1 )
	#break1
plt.xticks(np.arange(0, 601, 100),fontsize=20,fontname="Arial")
plt.yticks(np.arange(0., 0.71, 0.1),fontsize=20,fontname="Arial")
plt.xlim(xmin=0)
plt.xlabel('Simulation Time (s)',fontsize=22,fontname="Arial")
plt.ylabel('Activation ratio',fontsize=22,fontname="Arial" )#plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
plt.tight_layout()
plt.savefig('duration_figure_camkii.pdf')
...
for i,file in (enumerate([strength_files[x] for x in order])):
	...
	df = pd.read_csv(file,sep=sep)
	#empty_df[condition_names[i]] = (df['Values[CaMKII_active_ratio]'])
	plt.plot(df.index[::sample_rate]/1000,df['Values[PP2B_bar]'][::sample_rate],c=colors[i],label =sort_condition_names[i],linewidth=3,alpha=1)
	#break1
plt.xticks(np.arange(0, 601, 100),fontsize=20,fontname="Arial")
...
plt.ylabel('Activation ratio',fontsize=22,fontname="Arial" )#plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
plt.tight_layout()
plt.savefig('duration_figure_pp2b.pdf')
...
for i,file in (enumerate([strength_files[x] for x in order])):
	...
	# 	continue
	df = pd.read_csv(file,sep=sep)
	plt.plot(df.index[::sample_rate]/1000,df['Values[PP1a_bar]'][::sample_rate],c=colors[i],label =sort_condition_names[i],linewidth=3,alpha=1)
plt.xticks(np.arange(0, 601, 100),fontsize=20,fontname="Arial")
...
plt.ylabel('Activation ratio',fontsize=22,fontname="Arial" )#plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))
plt.tight_layout()
plt.savefig('duration_figure_pp1a.pdf')
...
legend.legend(*ax.get_legend_handles_labels(),loc='center', frameon=False,fontsize=15,labelspacing=0.7)
legend.savefig('duration_legend.pdf')